...
class TicTacToe: 
        ...
        if self.play:
            if(left_click) and self.__board.matrix[row][col] == INF:
                ...
                if(is_full == False):
                    ...
                    self.__board.print()
                    self.game_step += 1
                    best_situation = float('-inf') # player sẽ thắng
                    result = self.__botXAT.possibleMoves(self.__botXAT.play, self.__board) # lưu các bước đi có thể của bot
                    print("result: ", result)
                    for r in result:
                        self.__botXAT.action(r, self.__board) # đánh dấu vào bảng
                        eval = self.__botXAT.minimaxAlgorithm(False, self.depth, best_situation, float('inf'), self.__board) # mình con bot có thể win số dương
                    ...
                    self.__botXAT.action(move, self.__board) # tìm được bước đi tốt nhất
                    end_time = time.time()
    ...
    def winState(self): # ham nay ngoi code lai
    ...
    def drawObject(self):
        if(self.play == False): # nhan biet mot phat la thay doi luon
            self.switch = True
    ...
    def drawWiner(self): # ve duong thang sau do chuyen toi frame continue
        if(self.winer != None):
            self.game.drawLine(
...
class Game:
    ...
    def drawText(self, x, y, font, text, color): #ghi tu giua ra
